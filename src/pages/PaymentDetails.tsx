        ...
        <div className="p-6 space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900">{isEditing ? 'Edit Payment' : 'Payment Details'}</h1>
                        <p className="text-sm text-gray-600 mt-1">{payment.invoiceNumber}</p>
                    </div>
                </div>
            </div>
            ...
            {isEditing ? (
                ...
                <div className="space-y-6 max-w-4xl">
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        <h3 className="text-lg font-semibold text-gray-900 mb-6">Edit Payment Details</h3>
            ...
            <div className="space-y-6 max-w-6xl">
                <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-8">
                    <div className="flex justify-between mb-6">
                        <div>
                            <h3 className="text-2xl font-bold">{payment.clientName}</h3>
                            <p className="text-lg text-gray-700">{payment.projectName}</p>
                        </div>
                        <span className={`px-4 py-2 rounded-full text-sm font-semibold h-fit ${getStatusColor(payment.status)}`}>
                    ...
                    <div className="grid grid-cols-3 gap-6">
                        ...
                        <div className="bg-white bg-opacity-60 rounded-lg p-4">
                            ...
                            <p className="text-lg font-semibold">{new Date(payment.date).toLocaleDateString()}</p>
                        </div>
                        <div className="bg-white bg-opacity-60 rounded-lg p-4">
                ...
                <div className="grid grid-cols-2 gap-8">
                    <div className="bg-white border rounded-xl p-6">
                        <h4 className="text-lg font-bold mb-4">💳 Payment Information</h4>
                        <div className="space-y-4">
                            <div><p className="text-sm text-gray-600">Payment Method</p><p className="font-medium">{payment.paymentMethod || 'Not specified'}</p></div>
                            <div><p className="text-sm text-gray-600">Status</p><span className={`px-3 py-1 rounded-full text-sm ${getStatusColor(payment.status)}`}>{payment.status.toUpperCase()}</span></div>
                            <div><p className="text-sm text-gray-600">Transaction Date</p><p className="font-medium">{new Date(payment.date).toLocaleDateString()}</p></div>
                        </div>
                    </div>
                    <div className="bg-white border rounded-xl p-6">
                        <h4 className="text-lg font-bold mb-4">📊 Installment Progress</h4>
                        <div className="space-y-4">
                            <div><p className="text-sm text-gray-600">Current Installment</p><p className="text-2xl font-bold text-blue-600">{payment.installment}</p></div>
                            <div><p className="text-sm text-gray-600">Remaining</p><p className="font-medium">{payment.remaining}</p></div>
                            <div>
                ...
                {payment.description && (
                    <div className="bg-white border rounded-xl p-6">
                        ...
                        <div className="bg-gray-50 rounded-lg p-4"><p>{payment.description}</p></div>
                    </div>
                )}
                ...
                <div className="bg-white border rounded-xl p-6">
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        <div><p className="text-sm text-gray-600">Invoice Number</p><p className="font-medium">{payment.invoiceNumber}</p></div>
                        <div><p className="text-sm text-gray-600">Issue Date</p><p className="font-medium">{new Date(payment.date).toLocaleDateString()}</p></div>
                        <div><p className="text-sm text-gray-600">Client Name</p><p className="font-medium">{payment.clientName}</p></div>
                        <div><p className="text-sm text-gray-600">Project Name</p><p className="font-medium">{payment.projectName}</p></div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-4 gap-4">
                    ...
                    >
                     ...
                     <button className="px-4 py-3 bg-purple-50 text-purple-600 rounded-lg hover:bg-purple-100">📧 Email Invoice</button>
